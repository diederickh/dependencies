...
if [ ! -f ./dependencies.sh ] ; then
    echo "Create ${PWD}/dependencies.sh with the list of libraries you want to build."
    exit
fi
...
if [ "${build_glad}" = "y" ] ; then 
    if [ ! -f ${bd}/src/glad.c ] ; then
        ...
        if [ -f /usr/bin/python2 ] ; then
            ...
            python -m glad --generator=c --out-path=${sd}/glad/gl
        fi
...
# Compile nanovg (commented for now)
if [ "${build_nanovg}" = "y" ] ; then
